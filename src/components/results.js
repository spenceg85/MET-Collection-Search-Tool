...
export default function SearchResults(props) {
  ...
  const getListFromResults = (result, index) => {
    ...
    return (
        ...
        className={classes.title}
          primary={<Link href={result.package.links.npm} target="_blank" color="inherit">
          <h4>{result.package.name}</h4></Link>}
          secondary={
            <React.Fragment>
              <Typography
                component="span"
    ...
    const mapChips = (keyword) => {
        ...
        return (
            <Chip label={keyword} component="a" href={url} target="_blank" clickable />
        )
    }
    ...
    const mapScores = (detail) => {
        ...
        return (
            ...
            q<LinearProgress className={classes.quality} color="primary" variant="determinate" value={qualityScore} />
            m<LinearProgress className={classes.maintenance} color="primary" variant="determinate" value={maintenanceScore} />
            </div>
        )
    }
  ...
  return (
    <List className={classes.root}>
        {results && results.length ? results.map((node, index) => 
